...
public class UserController {
	...
	public ResponseEntity<String> updateUser(@PathVariable UUID id, @PathVariable String path, @RequestBody User user) {
		try {
			User currUser = getUserById(id);
   
			if (path.equalsIgnoreCase("address")) {
				userService.patchAddress(currUser, user);
			} else if (path.equalsIgnoreCase("emergencyContact")) {
